...
async def main():
    ...
    logger.info(f"Seeding {len(KNOWLEDGE_ENTRIES)} knowledge entries...")

    # Clear existing entries
    async with engine.begin() as conn:
        await conn.execute(
            __import__("sqlalchemy").text("DELETE FROM knowledge_base")
        )

    success = 0
    for category, title, content in KNOWLEDGE_ENTRIES:
        ok = await add_knowledge(category, title, content)
        if ok:
            success += 1
        else:
